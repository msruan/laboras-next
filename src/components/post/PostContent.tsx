...
export function PostContent({ userId, post, fullPage, onClick }: Props) {
  ...
  return (
    ...
    >
      ...
      {fullPage && (
        <footer className="mt-10 flex items-center border-t-purple-50 text-white text-xs opacity-70 max-sm:flex-col max-sm:gap-5">
          <p className="w-full">
            Data de publicação: {new Date(post.createdAt).toLocaleString()}
          </p>
          <div className="flex h-fit w-full flex-row justify-between pr-7 pb-1 sm:w-1/6">
